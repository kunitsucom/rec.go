...
func TestTime(t *testing.T) {
	...
	t.Run("success(0000)", func(t *testing.T) {
		...
		t.Cleanup(func() { CustomTimeFormat = backup })
		CustomTimeFormat = "1504"
...
func TestObject(t *testing.T) {
	...
	t.Run("error(unsupported)", func(t *testing.T) {
		defer func(backup *Logger) {
			defaultLogger = backup
		}(defaultLogger)

		buf := bytes.NewBuffer(nil)
		defaultLogger = Must(New(buf, WithUseTimestampField(false), WithUseCallerField(false)))
