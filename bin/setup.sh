...
test -d "${local_bin_dir:?}" || RecExec mkdir -p "${local_bin_dir:?}"
test -d "${local_tmp_dir:?}" || RecExec mkdir -p "${local_tmp_dir:?}"

if ! test -f "${pre_push_dst_path:?}"; then
...
if ! command -v golangci-lint >/dev/null; then
  ...
  RecExec bash -c "cd ${local_tmp_dir:?} && curl -fLRSs ${url:?} -o ./golangci-lint.tar.gz && tar xfz ./golangci-lint.tar.gz && mv -fv golangci-lint-*/golangci-lint ${local_bin_dir:?}"
fi
